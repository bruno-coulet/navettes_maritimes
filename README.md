...
Fusionne les registres maritimes et les données météo pour entraîner le Random Forest final à 146 features.
...
│
├── v2_meteo/         # Pipeline ML dédié exclusivement aux données environnementales
│   ├── src/          # Scripts chaînés (a_collect, b_consolidate, c_features, d_train, pipeline)
│   ├── data/         # Données brutes (CSV) et consolidées/splittées (Parquet)
│   └── artifacts/    # Modèle V2 et métriques
│
...
2. v2_meteo (Pipeline Météo & Entraînement)
Ce module gère le stockage et l'acquisition des données environnementales passées, et entraîne un modèle intermédiaire. Il interroge les API Open-Meteo (Marine et Archive) pour reconstituer l'historique (vagues, vent, rafales). Le pipeline gère la sauvegarde en CSV bruts, leur consolidation en format Parquet (haute performance), la fusion avec les annulations (corrigées sur la moyenne quotidienne), et l'entraînement de la V2.
...
Développé par Bruno Coulet — bruno.coulet@laplateforme.io