...
**Résultat:**
- Image: `open_meteo:latest` (~600MB avec dépendances ML)
...
docker run --rm -v $(PWD)/data:/app/data open_meteo:latest
...
docker stop open_meteo-collect
...
```bash
docker run -v $(pwd)/data:/app/data open_meteo:latest
```
