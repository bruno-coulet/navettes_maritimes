...
def convert_datetime_columns(df):
    """
    Convertit les colonnes 'Date' et 'Heure' en objets datetime, et extrait des informations utiles.
    """
...
def mistral(angle_deg):
    """
    Prend un angle en degrés et retourne 0 ou 1 si l'angle correspond au secteur Ouest à Nord (mistral 270°–360° ou 0°).
    """
    # 0 => pas du mistral
    if angle_deg is None or np.isnan(angle_deg):
        return 0
    
    # Normalisation de l’angle pour qu’il soit dans l’intervalle [0, 360[     
    angle_deg = angle_deg % 360

    return int(270 <= angle_deg <= 360 or angle_deg == 0)
    # vérifie si l’angle est dans l’intervalle [270°, 360°] ou exactement 0° => True ou False
    # Convertie en entier : 1 = True, 0 = False


def add_mistral_flag(df, col_prefix):
    """
    Ajoute une colonne binaire '{col_prefix}_mistral' si '{col_prefix}_deg' (c'est à dire 'vent', 'houle') correspond à du mistral.
    """
    deg_col = f'{col_prefix}_deg'
    # Recalcule l'angle à partir des composantes x et y (pour robustesse)
