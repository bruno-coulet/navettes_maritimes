"""
Script d'entraînement du modèle de prédiction d'annulation de navettes maritimes.

Charge les données historiques (maritime_clean.csv), entraîne un modèle RandomForest,
et exporte modèle + metadata pour inférence.
...
def load_and_prepare_data(csv_path: str | None = None) -> tuple[pd.DataFrame, pd.Series]:
    ...
    """
    if csv_path is None:
        csv_path = Path(__file__).resolve().parents[2] / "data" / "maritime_clean.csv"
    ...
    # Préparation : appliquer le pipeline de préprocessing
