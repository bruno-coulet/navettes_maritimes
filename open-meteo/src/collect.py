...
Responsabilite:
- interroger les APIs Open-Meteo pour la mer et la meteo generale
...
- `uv run -m meteo_marine.src.collect`
- `python -m meteo_marine.src.collect`
"""

from datetime import datetime, timedelta

try:
    # Cas package: python -m meteo_marine.src.collect
    from .utils import MeteoMarineMarseille
except ImportError:
    try:
        # Cas dossier meteo_marine comme racine: python -m src.collect
        from src.utils import MeteoMarineMarseille
    except ImportError:
        # Cas exécution directe du fichier: python meteo_marine/src/collect.py
        from utils import MeteoMarineMarseille
...
def test_api_connection(timeout=15):
    """Teste rapidement la connectivité aux APIs Open-Meteo utilisées par le pipeline."""
    print("=== TEST CONNECTIVITÉ API OPEN-METEO ===")
