...
|---------|------|-------------|--------|--------|------|
| **meteo_marine_collect** | Collecte données brutes | `~/projets/navettes_maritimes/meteo_marine/` | Open-Meteo API | `data/processed/` | — |
...
1. **`meteo_marine_collect`** : Récupère la météo via Open-Meteo → `meteo_data:/app/data`
...
Si le réseau de l'entreprise bloque l'accès direct à Open-Meteo, ajoutez vos variables
